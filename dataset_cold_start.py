...
#             except:
#                 pass
        all_movies = json.load(open('all_movies.json','r'))
        self.all_handled_movies = []
        for movie in list(set(all_movies)):
            try:
                en = self.id2entity[str(movie)]
                movie_id = self.entity2entityId[en]
                self.all_handled_movies.append(movie_id)
            except:
                pass
...
#             all_key_words = all_key_words[:int(0.4 * len(all_key_words))]     
            ...
            num_movies = 0
            for entity in line['entity']:
                if entity in self.all_handled_movies:
                    num_movies +=1
...
#                 #####
#                 try:
#                     if altitude[str(word[1:])]['liked'] == 1:                
