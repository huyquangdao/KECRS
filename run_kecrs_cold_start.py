...
from dataset_cold_start import dataset, CRSdataset
from kecrs_model import CrossModel
...
class TrainLoop_fusion_rec:
    ...
    def metrics_cal_rec(self, rec_loss, scores, labels, num_movies):
        ...
        for b in range(batch_size):
            if labels[b].item() == 0 or num_movies[b] > 0:
                continue
    ...
    def val(self, is_test=False):
        ...
        for (
            ...
            # print(losses)
            # exit()
            self.metrics_cal_rec(rec_loss, rec_scores, movie, num_movies)
    ...
    def train(self):
        ...
        for i in range(self.epoch * 3):
            ...
            for (
                ...
                joint_loss = gen_loss + self.lambda_1 * selection_loss + self.lambda_2 * alignment_loss

                losses.append([gen_loss, alignment_loss, selection_loss])
                self.backward(joint_loss)
                self.update_params()
                if num % 50 == 0:
                    print(
                    ...
                    print(
                        "alignment loss is %f" % (sum([l[1] for l in losses]) / len(losses))
                    )
                    losses = []
                num += 1
